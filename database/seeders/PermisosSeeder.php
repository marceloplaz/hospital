...
// 7. Obtener IDs para los roles de apoyo (Chofer, Manual, Enfermero)
$rolChofer = DB::table('roles')->where('nombre', 'Chofer')->first();
$rolPersonalManual = DB::table('roles')->where('nombre', 'Personal Manual')->first();
$rolEnfermero = DB::table('roles')->where('nombre', 'Enfermero')->first();

$rolesApoyo = [$rolChofer, $rolPersonalManual, $rolEnfermero];

// Definimos los permisos: Ver personal, Ver sus Turnos, Ver Servicios y Descargar
$permisosLecturaCompleta = DB::table('permisos')
    ->whereIn('nombre', [
        'ver-personas',    // Para ver la lista de personal
        'ver-turnos',      // Para ver a qué turno pertenecen
        'ver-servicios',   // Para ver a qué servicio pertenecen
        'descargar-pdf'    // Para bajar su información
    ])
    ->pluck('id');

foreach ($rolesApoyo as $rol) {
    if ($rol) {
        // Limpiamos relaciones previas
        DB::table('rol_permiso')->where('rol_id', $rol->id)->delete();
        
        // Asignamos los permisos seleccionados
        foreach ($permisosLecturaCompleta as $permisoId) {
            DB::table('rol_permiso')->insert([
                'rol_id' => $rol->id,
